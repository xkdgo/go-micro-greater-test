...
func (c *sayService) Goodbye(ctx context.Context, in *Request, opts ...client.CallOption) (*Response, error) {
	req := c.c.NewRequest(c.name, "Say.Goodbye", in)
	out := new(Response)
	err := c.c.Call(ctx, req, out, opts...)
	if err != nil {
		return nil, err
...
func RegisterSayHandler(s server.Server, hdlr SayHandler, opts ...server.HandlerOption) error {
	type say interface {
		...
		Goodbye(ctx context.Context, in *Request, out *Response) error
	}
	type Say struct {
		say
	}
...
func (h *sayHandler) Goodbye(ctx context.Context, in *Request, out *Response) error {
	return h.SayHandler.Goodbye(ctx, in, out)
}
